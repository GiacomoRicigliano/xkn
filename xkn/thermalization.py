...
class Thermalization(object):
    ...
    def __init__(self,therm_model):
        if   therm_model in ['BKWM', 'BKWM_dens']:
            ...
            # define the interpolation functions
            self.fa_1d = interp1d(x_barnes,a_barnes,bounds_error=False,fill_value=(a_barnes[0],a_barnes[-1]))
            self.fb_1d = interp1d(x_barnes,b_barnes,bounds_error=False,fill_value=(b_barnes[0],b_barnes[-1]))
            self.fd_1d = interp1d(x_barnes,d_barnes,bounds_error=False,fill_value=(d_barnes[0],d_barnes[-1]))
...
def BKWM_therm_efficiency(cls, **kwargs):
    if any( [kwargs['times'] is None, kwargs['omegas'] is None, kwargs['mass_ej'] is None, kwargs['vel'] is None] ):
        sys.exit('Error. For thermalization efficiency, user must specify times, angles, mass and velocity.\n')
    ...
    tmp           = 2. * coeffs[1] * times_days**coeffs[2]
    tmp           = 0.36 * (np.exp(-coeffs[0] * times_days) + np.log(1. + tmp) / tmp)
    return tmp.T

def power_law_therm_efficiency(cls, **kwargs):
    if any( [kwargs['times'] is None, kwargs['cnst_eff'] is None, kwargs['idx_eff'] is None] ):
        sys.exit('power_law_therm_efficiency: Error, for power-law thermalization efficiency, user must specify times, thermalization constant and thermalization index.\n')
    return kwargs['cnst_eff'] / kwargs['times']**kwargs['idx_eff']

def cnst_therm_efficiency(cls,  **kwargs):
    if kwargs['cnst_eff'] is None or kwargs['cnst_eff'] < 0. or kwargs['cnst_eff'] > 1.:
        sys.exit('cnst_therm_efficiency: Error, parameter "cnst_eff"={}. Cannot be None and has to be inside [0,1].\n'.format(kwargs["cnst_eff"]))
