...
class MKNConfig:
    ...
    def read_params(self):
        ...
        for section in self.config.sections():
            if 'vars' in section: continue
            tmp = {}
            for key in self.config[section]:
                if   self.config[section][key] == 'None':      tmp[key] = None
                elif params_info_dict[key][0] == 'bool':       tmp[key] = self.config[section].getboolean(key)
                elif params_info_dict[key][0] == 'float':      tmp[key] = float(self.config[section][key])
                elif params_info_dict[key][0] == 'int':        tmp[key] =   int(self.config[section][key])
                elif params_info_dict[key][0] == 'str':        tmp[key] =       self.config[section][key]
                elif params_info_dict[key][0] == 'int_list':   tmp[key] = [int(val) for val in self.config[section][key].split(' ')]
                elif params_info_dict[key][0] == 'float_list': tmp[key] = [float(val) for val in self.config[section][key].split(' ')]

            if section == 'glob': self.glob_params           = deepcopy(tmp)
            else:                 self.shell_params[section] = deepcopy(tmp)
    ...
    def init_vars(self):
        ...
        self.vars_secs  = {key[:-5]:self.config[key] for key in self.config.sections() if 'vars' in key}
        ...
        for comp, vars_sec in self.vars_secs.items():
            for key,val in vars_sec.items():
                if val == 'None': self.vars_fixed[comp][key] = None
                else:
                    try:
                        self.vars_fixed[comp][key]     = float(val)
                    except ValueError:
                        try:
                            self.vars_fixed[comp][key] = self.config[f'{comp}_vars'].getboolean(key)
                        except ValueError:
                            self.vars_free[comp][key]  = self._correct_ext_vars_key(val, key)
    ...
    def _correct_ext_vars_key(self, var, comp):
        import numpy as np
        if var in ['cos_iota', 'cosi']: return (var, np.arccos)
        else:                           return (var, self._ident)
    ...
    def get_vars(self, params):
        return {comp : {**{var : val[1](params[val[0]]) for var,val in self.vars_free[comp].items()},
...
def params_info():
    ...
    print('---------------------------------------------------------------------------------------------------------------------------')
    print()
    max_len = np.amax(np.array( [len(key) for key in {**glob_params_info_dict, **comp_vars_info_dict}] ))
...
def vars_info():
    ...
    print('---------------------------------------------------------------------------------------------------------------------------')
    print()
    max_len1 = np.amax(np.array( [len(key)    for key in {**glob_vars_info_dict, **comp_vars_info_dict}] ))
    max_len2 = np.amax(np.array( [len(val[0]) for val in {**glob_vars_info_dict, **comp_vars_info_dict}.values()] ))
...
glob_params_info_dict = {
    ...
    'filter_usage'          : ['str',       'import data and correspondent filter dictionary (measures) or not (properties) [properties, measures]'],
    'filter_dictionary'     : ['str',       'dictionary used for the filters [telescopes, iso_calc, AT2017gfo, lsst]'],
    'filter_dictionary_path': ['str',       'path to the filter dictionary: if None, uses the default one to telescopes'],
    ...
    # dered correction
    'dered_correction'      : ['bool',      'correction for reddening: if True uses filters.dered_CCM to correct data for reddening.'],
    ...
    'slices_dist'           : ['str',       'discretization law for the polar angle [uniform, cos_uniform]'],
    'omega_frac'            : ['float',     'auxiliary parameter [1]'],
    # times handling
    't_scale'               : ['str',       'scale for the velocity [lin, log, measures] - measures will use data times'],
    't_num'                 : ['int',       'integer number of bins in time'],
    't_min'                 : ['float',     'minimum time in s (has to be bigger than 1e4 s (0.12 d) for Ricigliano and Composed model)'],
    't_max'                 : ['float',     'maximum time in s'],
    't_start_filter'        : ['float',     'start time of filter data in respective units (e.g. Julian days for AT2017gfo)'],
    't_toll'                : ['float',     'relative tolerance between time steps when extracting from measures'],
    # vel handling
    ...
    'thin_shells'           : ['bool',      'if True applies the opcitally thin shells correction (only for ricigliano_lippold model)'],
    'n_thin'                : ['int',       'number of optically thin shells'],
    'shell_const'           : ['str',       'uniform discretization in mass (mass) or velocity (vel) for optically thin shells'],
    'n_heat'                : ['int',       'auxiliary parameter [30]'],
    }

comp_params_info_dict = {
    'mass_dist'             : ['str',       'mass angular distribution law [uniform, sin, sin2, cos2, step]'],
    'vel_dist'              : ['str',       'velocity angular distribution law [uniform, sin, sin2, cos2, abscos, step]'],
    'op_dist'               : ['str',       'opacity angular distribution law [uniform, sin, sin2, cos2, abscos, step]'],
    'therm_model'           : ['str',       'thermalization model [BKWM, BKWM_dens, BKWM_1d, power_law, cnst]'],
...
comp_vars_info_dict = {
    ...
    'step_angle_mass'       : ['float',     'angle of the step in the mass distribution in radians'],
    ...
    'step_angle_vel'        : ['float',     'angle of the step in the velocity distribution in radians'],
    ...
    'step_angle_op'         : ['float',     'angle of the step in the opacity distribution in radians'],
