...
class Kappa2Ye(object):
    ...
    def __init__(self, ye_k_dep):
        ...
        else: sys.exit('Wrong input name for ye-k dependence\n' +
                       'Please use:\n' +
                       '"TH" for Tanaka & Hotokezaka 2020\n')

    def __call__(self, mode, value):
        if mode == 'opacity': return self.f_k2ye(value) # calculate from value of opacity
        elif mode == 'ye':    return self.f_ye2k(value) # calculate from value of ye
        else: sys.exit("Please choose 'opacity' or 'ye' to convert from one to the other")
