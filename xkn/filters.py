...
def read_filter_measures(
    ...
    if not os.path.exists(filter_data_path):
        raise FileNotFoundError(
            f"The path to the filter data {filter_data_path} does not exist!"
        )
    ...
    for lam in lams:
        ...
        if dered_correction:
            mags_tot -= dered_CCM(np.asarray([lam]), R_V=R_V, EBV=EBV, A_V=A_V)
...
def dered_CCM(wave, R_V=3.1, EBV=0.105, A_V=None):
    ...
    """
    x = 1000.0 / wave  # Convert to inverse microns
    a = np.zeros_like(x)
    b = np.zeros_like(x)
    ...
    if np.any(mask):
        ...
        a[mask] = (
            1.752 - 0.316 * x[mask] - (0.104 / ((x[mask] - 4.67) ** 2 + 0.341)) + F_a
        )
        b[mask] = (
            -3.090 + 1.825 * x[mask] + (1.206 / ((x[mask] - 4.62) ** 2 + 0.263)) + F_b
        )
    ...
    return (
        np.sum(
            np.array(
                [
                    lum * f
                    for lum, f in zip(lumbol, flux_factors[: len(flux_factors) // 2])
                ]
            )
            + np.array(
                [
                    lum * f
                    for lum, f in zip(
                        lumbol[::-1], flux_factors[len(flux_factors) // 2 :]
                    )
                ]
            ),
            axis=0,
...
def calc_lum_iso_from_mags(mags, dic_filt, distance):
    lambdas = [100.0 * dic_filt[lam]["lambda"] for lam in mags]
    lum_lams = np.array(
        [
            10 ** ((mags[lam]["mag"] + 48.6) / -2.5)
            * utils.fourpi
            * (distance * utils.Mpc2cm) ** 2
            / (100.0 * dic_filt[lam]["lambda"]) ** 2
            * utils.c
            for lam in mags
        ]
    )
...
def planckian(nu, T_plk):
    return (
        ...
        * nu**3
        / utils.c2
        / planckian_help(utils.h * nu / utils.kB / T_plk)
    )
...
def calc_fnu(
    ...
    if T_photo is not None:
        fnu_cont += np.where(
            radius_photo > 0,
            radius_photo**2
            * planckian(utils.c / (100.0 * lambda_meters / (1.0 + redshift)), T_photo),
            0,
        )
    if lum_shells is not None and T_shells is not None and omegas is not None:
        ...
        fnu_cont_tmp[mask_zeroes] = (
            lum_shells
            / (omegas[None, :, None, None] * utils.sigma_SB * T_shells**4)
            * planckian(utils.c / (100.0 * lambda_meters / (1.0 + redshift)), T_shells)
        )[mask_zeroes]
        fnu_cont += fnu_cont_tmp.sum(axis=-1).sum(axis=0)
    return (
        (
            fnu_cont.T * flux_factors[: len(flux_factors) // 2][None, :]
            + fnu_cont[::-1].T * flux_factors[len(flux_factors) // 2 :][None, :]
        ).sum(axis=-1)
        * (1.0 + redshift)
        / distance**2
    )
...
def calc_residuals(
    ...
    **kwargs,
    ...
    return {
        lam: mag_diffs[lam]
        / np.sqrt(
            prep_sigma(mag[lam]["sigma"], mag_diff=mag_diffs[lam]) ** 2 + sigma_sys**2
        )
        for lam in lams
    }
...
def limit_lams(dic_filt, lams, mag, lam_list=None, lam_min=None, lam_max=None):
    ...
    if lam_list is None and lam_min is None and lam_max is None:
        return dic_filt, lams, mag
    else:
        if lam_min is None:
            lam_min = 0
        if lam_max is None:
...
def limit_mags(dic_filt, lams, mag, mag_min=None, mag_max=None):
    ...
    if not mag or (mag_min is None and mag_max is None):
        return dic_filt, lams, mag
    elif mag_min is None:
        mag_min = -np.inf
    ...
    for lam in lams:
        ids = np.nonzero(
            np.logical_and(mag[lam]["mag"] >= mag_min, mag[lam]["mag"] <= mag_max)
        )[0]
        if not len(ids):
            del dic_filt[lam]
            del mag[lam]
    ...
    if not injection_mag:
        ...
        for lam in injection_mag:
            if len(injection_mag[lam]["mag"]):
                ...
                injection_mag[lam]["sigma"] = rng.uniform(
                    sigma_min, sigma_max, len(new_mags[lam]["time"])
                )
