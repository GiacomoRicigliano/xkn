...
def importNRprofiles(filename, angles):
    ...
    dth        = th[0]
    if dth == 0: sys.exit('NR data unsupported format: input central values for angular bins')
    ...
    profile_m *= np.sum(mass_smooth)/np.sum(2.*profile_m)
...
def smooth_array(arr):
    ...
    arr_smooth[0]    =  larr[0]
    arr_smooth[1]    = (larr[0]+larr[1]+larr[2])/3.
    arr_smooth[2]    = (larr[0]+larr[1]+larr[2]+larr[3]+larr[4])/5.
    arr_smooth[3:-3] = (larr[:-6]+larr[1:-5]+larr[2:-4]+larr[3:-3]+larr[4:-2]+larr[5:-1]+larr[6:])/7.
    arr_smooth[-3]   = (larr[-5]+larr[-4]+larr[-3]+larr[-2]+larr[-1])/5.
    arr_smooth[-2]   = (larr[-3]+larr[-2]+larr[-1])/3.
    arr_smooth[-1]   =  larr[-1]
    return 10.**arr_smooth
